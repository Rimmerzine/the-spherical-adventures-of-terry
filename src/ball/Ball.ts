...
class Ball implements PositionedObject {
    ...
    draw(context: CanvasRenderingContext2D, offsetX: number, offsetY: number): void {
        ...
        // Draw the pupil
        context.beginPath();
        context.arc(drawPositionX + eyeOffsetX + pupilOffsetX, drawPositionY - eyeOffsetY, pupilRadius, 0, 2 * Math.PI);
    ...
    move(deltaTime: number): void {
        this.position = this.position.add(this.attributes.velocity.multiply(this.attributes.remainingMovement).multiply(deltaTime));
    }
